...
//            sort point data into each box  (see helper function getMeshPonitsInBox())
...
void Octree::subdivide(const ofMesh& mesh, TreeNode& node, int numLevels, int level) {
	...
	for (int i = 0; i < boxList.size(); i++) {
		child.box = boxList[i];
		int p = getMeshPointsInBox(mesh, node.points, child.box, child.points);
		if (p > 0) {
...
bool Octree::intersectNodeList(const Box& box, TreeNode& node, vector<TreeNode>& nodeListRtn) {
	bool intersects = false;
	if (node.box.overlap(box)) {
		if (node.children.size() > 0) {
			for (int i = 0; i < node.children.size(); i++) {
				if (intersectNodeList(box, node.children[i], nodeListRtn)) {
					intersects = true;
				}
			}
		}
...
glm::vec3 Octree::getAverageNormal(vector<TreeNode>& nodeList) {
	if (nodeList.size() == 0) {
		return glm::vec3(0, 0, 0);
	}
	glm::vec3 averageNormal;
	for (int i = 0; i < nodeList.size(); i++) {
		averageNormal += mesh.getNormal(nodeList[i].points[0]);
	}
	averageNormal /= nodeList.size();
	averageNormal = glm::normalize(averageNormal);
	return averageNormal;
}

glm::vec3 Octree::getAverageBoxCenter(vector<Box>& colBoxList) {
	...
	for (int i = 0; i < colBoxList.size(); i++) {
		...
		averageCenter += glm::vec3(center.x(), center.y(), center.z());
	}
	averageCenter /= colBoxList.size();
