...
glm::vec3 TurbulenceForce::getForce() {
	return glm::vec3(ofRandom(tmin.x, tmax.x), ofRandom(tmin.y, tmax.y), ofRandom(tmin.z, tmax.z));
}
...
void ImpulseRadialForce::updateForce(Particle * particle) {
	...
	ofVec3f dir = ofVec3f(ofRandom(-1, 1), ofRandom(-1, 1), ofRandom(-1, 1)).getNormalized();
	ofVec3f f = ofVec3f(dir.x * ofRandom(magnitude / 2, magnitude), 0, dir.z * ofRandom(magnitude / 2, magnitude));
	particle->forces += f;
}
